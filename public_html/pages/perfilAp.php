...
<div class='card'>
<div class='card-body'>
  <h5 class='card-title'>Apartamento <?php echo $result['apartamento']. ($result['locado'] == 1 ? ' - Locado' : ' - Vago'); ?> </h5>
  <p class='card-text'>
    <div class='container'>
<div class='row'>
<div class='col-md-3'>
  <img src='\img\residential.png'  class='img-fluid' alt='Foto do apartamento'>
</div>
<div class='col-md-9'>
  ...
  <button type='submit' id='saveBtn' class='btn btn-outline-secondary float-right' style='display: none;'><i class='fa-regular fa-circle-check'></i>
  </i></button>
...
<input type='hidden' name='id_apartamento' value="<?php echo $id ?>" required>
  ...
  <hr>
    ...
    <form id='inquilinoForm' action='/public_html\control\atualizar_inquilino.php' method='post'
    style=" <?php if ($result['locado'] == 0) {
    echo " display:none;";}?>">
...
<?php if ($result !== false) { echo"
    <form id='retirar_inquilino_form' action='\public_html\control"; ?>\retirar_inquilino.php'<?php echo" method='post'>
